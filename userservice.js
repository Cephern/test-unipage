...
${'#login'}.click(() => {
    ...
    // Конструктор класса нельзя вызывать без ключевого слова "new", поэтому сначала объявляем инстанс класса, а затем, 
    // так как в классе "UserService" метод authenticate_user - статичный, вызываем его на самом классе, передавая данные пользователя в качестве аргументов
...
// Вместо XML запроса я бы использовал fetch api или axios, чтобы обрабатывать эти запросы как промисы.
// Так же вместо метода "GET" я бы использовал "POST", а логин и пароль вложил в тело запроса.
